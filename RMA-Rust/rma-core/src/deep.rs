...
/// RMA Deep : mémoire relationnelle + réseau fully-connected en sortie
pub struct RelationalMemoryAccumulatorDeep {
    ...
    weights: Vec<Vec<Vec<f64>>>,  // weights[i][row][col]
    biases: Vec<Vec<f64>>,
}

impl RelationalMemoryAccumulatorDeep {
    /// Constructeur avec architecture MLP personnalisable
    pub fn new(
        input_dim: usize,
        hidden_sizes: Vec<usize>,
        output_dim: usize,
        ...
        layer_sizes.extend(hidden_sizes.iter().copied());
        ...
        for i in 0..layer_sizes.len() - 1 {
            ...
            let mut layer_b = vec![0.0; out_size];

            for row in &mut layer_w {
                for val in row {
    ...
    /// Étape principale : traitement + sortie via MLP
